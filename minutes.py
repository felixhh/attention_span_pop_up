...
def show_reminder(minutes):
    while True:
        ...
        reminder_label = ttk.Label(root, text=f"Time to focus for {minutes} minutes!\nClose this window to reset the timer.", background='red', font=('Arial', 12))
        reminder_label.pack(padx=20, pady=20)

        result = messagebox.showinfo("Reminder", f"Time to focus for {minutes} minutes!\nClose this window to reset the timer.")
        root.destroy()
...
def set_reminder():
    minutes = int(reminder_entry.get())
    if minutes > 0:
        show_reminder(minutes)
    else:
        messagebox.showwarning("Invalid Input", "Please enter a valid reminder time in minutes.")
...
reminder_label = ttk.Label(frame, text="Enter reminder time (minutes):")
